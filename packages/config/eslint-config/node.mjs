...
export default [
  ...
  // 3) Tests
  {
    ...
    files: ['**/*.spec.ts', '**/*.e2e-spec.ts', 'test/**/*.{ts,js}'],
    rules: {
      '@typescript-eslint/no-explicit-any': 'off',
      'import/order': 'off'
    }
  ...
  // 4) Seeds & factories
  {
    ...
    files: ['src/database/seeds/**/*.ts', 'src/database/factories/**/*.ts'],
    rules: {
      '@typescript-eslint/no-explicit-any': 'off',
      'import/order': 'off'
    }
  ...
  // 6) Infra one-offs that legitimately use `any`
  {
    ...
    files: ['src/database/data-source.ts', 'src/database/typeorm.config.service.ts', 'src/utils/discover-modules.ts'],
    rules: {
      '@typescript-eslint/no-explicit-any': 'off'
    }
  }
